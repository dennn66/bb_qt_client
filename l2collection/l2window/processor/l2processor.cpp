#include "l2processor.h"


L2Processor::L2Processor(HWND winhwnd, QObject *parent) : QObject(parent)
{
    hwnd = winhwnd;
    image_width = 0;
    image_height = 0;
...
bool L2Processor::isSkillRdy(int num){
    if(!isActiveWindow) return false;
    return isSkillConditionRdy(num);
}

bool L2Processor::isSkillConditionRdy(int num){
...
void L2Processor::capture(){
    qDebug("L2Processor::capture");
...
bool L2Processor::activateSettings(int index){
    if(!isValidIndex(index))return false;
    activeCondSet = index;
    return true;
}

KeyConditionsSet* L2Processor::getCurrentSettings(){
    if(!isValidIndex(activeCondSet))return NULL;
    return cond_set_list[activeCondSet];
}

int L2Processor::LoadProject(QString file_name){
...
int L2Processor::SaveProject(QString file_name){
...
int L2Processor::AddConfig(QString file_name){
...
int L2Processor::LoadConfig(QString file_name){
    qDebug("L2Processor::LoadConfig");
    getCurrentSettings()->LoadConfig(file_name);
    return status;
}

int L2Processor::SaveConfig(QString file_name){
...
int L2Processor::getXP(int index){
    if(index < idMobHP) {
 ...
 void L2Processor::getStatusBtn(QImage* imgStatus, bool pressed){
    QPainter p;
